...
def send_message(handles)
  client = login_twitter
  5.times { client.update("Bonjour #{handles.sample}, j'adore ton travail !") }
end
...
def go_like_and_follow
  client = login_twitter
  client_stream = streaming_connection

  client_stream.filter(track: '#bonjour_monde') do |object|
    client.fav object # favorite the tweet
    client.follow object.user # follow the user who tweeted
  end
end
